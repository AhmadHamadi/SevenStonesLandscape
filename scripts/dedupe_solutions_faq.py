"""
Replace duplicate FAQPage schema blocks on solutions pages with unique, problem-specific Q&A.
Each page gets 6 new questions tailored to the specific problem, with Ontario-specific specifics
(clay soil, frost depth, OBC, city permits, real cost ranges). No city-name-only variation.
"""
...
# Unique FAQ content per slug. Each list = 6 Q&A pairs, all problem-specific.
...
FAQ_PREFIX = '<script type="application/ld+json">{"@context":"https://schema.org","@type":"FAQPage",'
...
def build_faq_block(qa_pairs: list[tuple[str, str]]) -> str:
    main_entity = [
        {
            "@type": "Question",
            "name": q,
...
def main() -> None:
    ...
    for slug, qa in FAQ_BY_SLUG.items():
        ...
        if FAQ_PREFIX not in html:
            print(f"[skip] no FAQPage block found: {slug}")
            continue
        new_block = build_faq_block(qa)
        new_html, count = FAQ_PATTERN.subn(new_block, html, count=1)
        if count == 0:
            print(f"[skip] pattern match failed: {slug}")
            continue
        path.write_text(new_html, encoding="utf-8")
        changed += 1
        print(f"[ok] rewrote {count} FAQ block(s) in {slug}")
